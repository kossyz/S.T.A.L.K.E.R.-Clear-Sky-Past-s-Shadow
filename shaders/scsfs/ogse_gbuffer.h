...
struct GB
{
	float4	_rt1	:	SV_Target0;
	float4	_rt2	:	SV_Target1;
	float4	_rt3	:	SV_Target2;
};
	...
	float3	get_xyz_position_fast	(float2 tc)	{return s_position.SampleLevel(smp_nofilter, tc, 0).xyz;}
	float3	get_xyz_position_proj	(float4 tc)	{return s_position.Sample(smp_nofilter, tc.xy / tc.w).xyz;}
	float	get_material			(float2 tc)	{return s_position.Sample(smp_nofilter, tc).w;}
	//float	get_xyz_hemi			(float2 tc)	{return s_normal.Sample(smp_nofilter, tc).w;}
	float	get_gloss				(float2 tc)	{return s_diffuse.Sample(smp_nofilter, tc).w;}
	...
	//float	get_depth_fast			(float2 tc)	{return s_position.SampleLevel(smp_nofilter, tc, 0, 0).z;}
	float	get_depth_fast			(float2 tc)	{return s_position.SampleLevel(smp_nofilter, tc, 0).z;}
	float	get_depth_proj			(float4 tc)	{return s_position.Sample(smp_nofilter, tc.xy / tc.w).z;}
	...
	float4	get_full_normal			(float2 tc)	{return decode_normal(s_normal.Sample(smp_nofilter, tc));}
	//float4	get_full_normal_fast	(float2 tc)	{return decode_normal(tex2Dlod(s_normal, float4(tc,0,0)));}
	float4	get_full_normal_fast	(float2 tc)	{return decode_normal(s_normal.SampleLevel(smp_nofilter, tc, 0));}
	float4	get_full_normal_proj	(float4 tc)	{return decode_normal(s_normal.Sample(smp_nofilter, tc.xy / tc.w));}
	...
	//float	get_refl_power			(float2 tc) {return s_normal.Sample(smp_nofilter, tc).z;}
#else
struct GB
{
	float4	_rt1	:	SV_Target0;
	...
	float4	get_full_normal			(float2 tc)	{return s_normal.Sample(smp_nofilter, tc);}
	float4	get_full_normal_proj	(float4 tc)	{return s_normal.Sample(smp_nofilter, tc.xy / tc.w);}
	float3	get_xyz_normal			(float2 tc)	{return s_normal.Sample(smp_nofilter, tc).xyz;}
	//float3	get_xyz_normal_fast		(float2 tc)	{return tex2Dlod(s_normal, float4(tc,0,0)).xyz;}
	float3	get_xyz_normal_fast		(float2 tc)	{return s_normal.SampleLevel(smp_nofilter, tc, 0).xyz;}
	float3	get_xyz_normal_proj		(float4 tc)	{return s_normal.Sample(smp_nofilter, tc.xy / tc.w).xyz;}
